...
impl WondwiseDirs {
    pub fn load() -> Self {
        if let Some(dirs) = ProjectDirs::from("com", "jheysonsaav", "wondwise") {
            Self {
...
mod tests {
    ...
    fn test_configdir_load() {
        ...
        assert_eq!(
            dirs.config_dir.to_str().unwrap(),
            format!(
                "{}/Library/Application Support/com.jheysonsaav.wondwise",
                env::var("HOME").unwrap()
            )
            .as_str()
        );
    ...
    fn test_datadir_load() {
        ...
        assert_eq!(
            dirs.data_dir.to_str().unwrap(),
            format!(
                "{}\\AppData\\Roaming\\jheysonsaav\\wondwise\\data",
    ...
    fn test_cachedir_load() {
        ...
        assert_eq!(
            dirs.cache_dir.to_str().unwrap(),
            format!(
                "{}\\AppData\\Local\\jheysonsaav\\wondwise\\cache",
                env::var("USERPROFILE").unwrap()
            )
            .as_str()
        );
        ...
        assert_eq!(
            dirs.cache_dir.to_str().unwrap(),
            format!(
                "{}/Library/Caches/com.jheysonsaav.wondwise",
                env::var("HOME").unwrap()
            )
            .as_str()
        );
